...
public class MidiPlayer
{
    ...
    /**
     * sets the instrument used to play the notes after the 'startTick'
     *
     * @param instrument   the integer value of an instrument
     * @param startTick    the starting time of the event in ticks
     * @param trackIndex   the index of the track to which the new event will be added
     */
    public void setInstrument(int instrument, int startTick, int trackIndex)
    {
        Track track = sequencer.getSequence().getTracks()[trackIndex];
        ShortMessage message = new ShortMessage();
        try {
            message.setMessage(ShortMessage.PROGRAM_CHANGE, 0, instrument, 0);
            MidiEvent instrumentChange = new MidiEvent(message, startTick);
            track.add(instrumentChange);
        } catch (InvalidMidiDataException e) {
            e.printStackTrace();
        }
    }
